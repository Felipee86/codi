...
abstract class Rendus_Layout_Abstract  implements Rendus_Interface
{
  ...
  private function loadSockets()
  {
    $q = "
          SELECT
            rs.name,
            source_rl.name AS component_class
          FROM
            rendus_socket rs
          JOIN
    ...
    foreach ($ASockets as $ASocket) {
      if (Codi_Class::loadClass($ASocket['component_class'])) {
  ...
  public final function getCss(&$index = 1)
  {
    ...
    foreach ($this->ACss as $cssFilename) {
      $cssFilePath = Codi_Request::getModule() . '/css/' . $cssFilename;
      $ACss += [
          'link#' . $index => [
              'attr' => [
                  'rel'  => 'stylesheet',
